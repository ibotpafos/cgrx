...
| Budgeted context bundles and project maps | `orient` + revision-bound `expand` return bounded task context; `get_architecture` adds packages, proven boundaries, hotspots, package cycles and deterministic communities | Architecture now adds exact repository-local `IMPORTS` and conservative static `REFERENCES` for all four supported language packs; external targets are omitted and ambiguous local targets remain coverage gaps. |
