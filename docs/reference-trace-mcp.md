...
| Budgeted context bundles and project maps | `orient` + revision-bound `expand` return bounded task context; `get_architecture` adds packages, proven boundaries, hotspots, package cycles and deterministic communities | Extend architecture only with independently verified relation kinds; the current projection explicitly reports `CALLS`/`IMPLEMENTS` and coverage gaps. |
