...
| Budgeted context bundles and project maps | `orient` + revision-bound `expand` return bounded task context; `get_architecture` adds packages, proven boundaries, hotspots, package cycles and deterministic weighted-modularity communities | Architecture uses exact repository-local `CALLS`, `IMPLEMENTS`, `IMPORTS` and conservative static `REFERENCES`; it reports modularity, cohesion, cut/internal weights and the relation weights so clustering quality stays inspectable. |
