...
at most 12 candidates with bounded text excerpts and accepts only a complete,
duplicate-free set of integer relevance scores from 0 to 1000. The score is a
small additional packing signal; structural evidence and conservative coverage
rules remain authoritative. When the sidecar is configured, `orient` also adds
`semantic_rerank` metadata with `status: applied` or `status: fallback`. The
field is omitted when semantic reranking is disabled.
