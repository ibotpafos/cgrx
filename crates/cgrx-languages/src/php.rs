    ...
    match node.kind() {
        ...
        "function_call_expression" => {
            ...
            let target = function
                ...
                .and_then(|function| facts.functions.get(&text(function, source).to_ascii_lowercase()))
            ...
            if !facts.blocked && !reference && inside_named_body(node)
        ...
        "named_type" => {
            edge(RelationKind::References, text(node, source), node, extraction);
        }
        _ => {}
    }
}
...
mod tests {
    ...
    fn extract(source: &str) -> Extraction {
        PHP_PACK.extract(Path::new("example.php"), source.as_bytes()).unwrap()
    }
    ...
    fn php_all_extensions_extract_exact_symbols_relations_and_dispatch_gaps() {
        for extension in PHP_PACK.extensions() {
            ...
            let result = PHP_PACK.extract(Path::new(&path), SOURCE.as_bytes()).unwrap();
            ...
            for name in ["helper", "caller", "dynamic"] {
                ...
                assert!(SOURCE[symbol.search_span.start..symbol.search_span.end].starts_with("function "));
            }
            for (relation, target, snippet) in [
                ...
                assert!(result.edges.iter().any(|edge| edge.relation == relation
                    && edge.target == target && &SOURCE[edge.span.start..edge.span.end] == snippet), "{path}: {result:?}");
            }
            assert_eq!(result.edges.iter().filter(|e| e.relation == RelationKind::Calls).count(), 1);
            assert!(result.unresolved.iter().any(|u| u.kind == UnresolvedKind::Dispatch
    ...
    fn php_dynamic_receiver_variable_static_and_callable_references_are_not_calls() {
        for body in ["$helper();", "$receiver->helper();", "$receiver?->helper();", "Service::helper();", "helper(...);", "(fn() => helper())();"] {
            let source = format!("<?php function helper() {{}} function caller($helper, $receiver) {{ {body} }}");
            let result = extract(&source);
            assert!(result.parser_error_ranges.is_empty(), "{body}: {result:?}");
            assert!(!result.unresolved.is_empty(), "{body}: {result:?}");
            assert!(!result.edges.iter().any(|e| e.relation == RelationKind::Calls), "{body}: {result:?}");
        }
    }
    ...
    fn php_ambiguous_conditional_namespaced_imported_and_missing_targets_abstain() {
        for source in [
            ...
            let result = extract(source);
            assert!(result.parser_error_ranges.is_empty(), "{source}: {result:?}");
            assert!(!result.unresolved.is_empty(), "{source}: {result:?}");
            assert!(!result.edges.iter().any(|e| e.relation == RelationKind::Calls), "{source}: {result:?}");
        }
    }
    ...
    fn php_function_names_are_ascii_case_insensitive_but_spans_keep_original_bytes() {
        ...
        let result = extract(source);
        let call = result.edges.iter().find(|e| e.relation == RelationKind::Calls).unwrap();
    ...
    fn php_group_imports_keep_each_binding_and_do_not_use_alias_as_target() {
        let result = extract("<?php use Vendor\\{Payload as Item, Other}; function caller(Item $item): Other {}");
        let imports: Vec<_> = result.edges.iter().filter(|e| e.relation == RelationKind::Imports).map(|e| e.target.as_str()).collect();
        assert_eq!(imports, vec!["Vendor\\Other", "Vendor\\Payload"]);
    }

    #[test]
    fn php_parser_errors_and_absolute_paths_fail_closed() {
        let result = extract("<?php function helper() {} function caller() { helper(); } function broken(");
        assert!(!result.parser_error_ranges.is_empty());
        assert!(!result.edges.iter().any(|e| e.relation == RelationKind::Calls));
        assert_eq!(PHP_PACK.extract(Path::new("/example.php"), b""), Err(ExtractError::AbsolutePath));
    }
}
