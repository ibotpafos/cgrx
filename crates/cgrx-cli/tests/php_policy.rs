...
fn php_registry_policy_has_separate_cache_identity_and_reindexes_the_same_commit() {
    let fixture = Fixture(std::env::temp_dir().join(format!(
        "cgrx-php-policy-{}-{}", std::process::id(), SEQUENCE.fetch_add(1, Ordering::Relaxed)
    )));
    ...
    fs::write(root.join("main.php"), "<?php function helper() {} function caller() { helper(); }\n").unwrap();
    for args in [
        vec!["init", "-q"], vec!["add", "."],
    ...
    assert_eq!(cgrx_languages::pack_for_path(Path::new("main.php")).is_some(), enabled);
    ...
    assert_eq!(report.snapshot.graph_generation, generation(&report.snapshot.repo_revision, active_policy));
    let reader = GenerationReader::open_current(&fresh).unwrap();
    let mut stored: Value = serde_json::from_slice(&reader.read_segment("nodes.seg").unwrap()).unwrap();
    assert_eq!(stored["extraction_revision"], active_policy);
    assert_eq!(stored["documents"].as_array().unwrap().iter().any(|doc| doc["path"] == "main.php"), enabled);
    assert!(stored["documents"].as_array().unwrap().iter()
        .filter(|doc| doc["path"] == "keep.rs").all(|doc| doc.get("php_function_target").is_none()));
    ...
    for name in ["edges.seg", "terms.fst"] {
        writer.write_segment(name, &reader.read_segment(name).unwrap()).unwrap();
    }
    writer.validate().unwrap();
    writer.publish().unwrap();
    let pinned = GenerationReader::open_current(&migrated).unwrap();
    assert_eq!(Runtime::open(&migrated).err().unwrap().code(), "extraction_revision");
    assert_eq!(Runtime::index(&root, &migrated).unwrap(), report);
    assert_eq!(Runtime::index(&root, &migrated).unwrap(), report);
    let runtime = Runtime::open(&migrated).unwrap();
    assert_eq!(runtime.coverage().excluded_paths.contains(&"main.php".to_owned()), !enabled);
