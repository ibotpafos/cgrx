...
fn is_callable_candidate(language: &str, source: &str) -> bool {
    ...
    match language {
        ...
        "c" => source.contains('(') && source.contains('{'),
        "kotlin" => tokens.iter().any(|token| token == "fun"),
        "typescript" => {
    ...
    pub fn suggest_refactors(
            ...
            || !(1..=50).contains(&limit)
        {
            return Err(RuntimeError::new(
                ...
                "language must be typescript, go, java, c, kotlin, python, or rust; min_score must be 0..1000; limit must be 1..50",
            ));
        }
