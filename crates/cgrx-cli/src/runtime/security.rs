...
mod tests {
    ...
    /// Minimal std-only temporary directory that removes itself on drop.
    struct TempDir {
    ...
    impl TempDir {
        fn new() -> Self {
            ...
            let path = base.join(format!("cgrx-sec-{}-{}-{}", std::process::id(), stamp, seq));
    ...
    fn untracked_symlink_is_not_followed_outside_the_repository() {
        ...
        fs::write(&external, "let api_key = \"external-secret-value\";\n").unwrap();
        symlink(&external, repository.path().join("outside-link")).unwrap();
